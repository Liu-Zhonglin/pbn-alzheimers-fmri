...
class ContextSpecificDecoder:
    ...
    def run(self):
        ...
        # --- NEW PROGRESS CHECK ---
        print("\nGenetic algorithm finished. Now inferring Boolean models from the best solutions...")
        boolean_models = []
        for i, front in enumerate(fronts):
            # --- NEW PROGRESS CHECK ---
            print(f"\nProcessing solution {i + 1}/{len(fronts)} from the Pareto front...")
            boolean_model = self.getBooleanModel(front)
            boolean_models.append(boolean_model)
