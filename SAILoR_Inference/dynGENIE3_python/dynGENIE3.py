    ...
    for (i, current_timeseries) in enumerate(TS_data):
        ...
        for j in range(ngenes):
            ...
            if xmin != xmax:
                ...
                # --- START OF FIX ---
                # Check for zero time difference to prevent division by zero
                time_diff = abs(tmin - tmax)
                if time_diff == 0:
                    alphas[i, j] = 0.0
                else:
                    ...
                    alphas[i, j] = (xmax - xmin) / time_diff
                # --- END OF FIX ---
...
def dynGENIE3_single(TS_data,time_points,SS_data,output_idx,alpha,input_idx,tree_method,K,ntrees,compute_quality_scores,save_models):
    ...
    for (i,current_timeseries) in enumerate(TS_data):
        ...
        time_diff_current[time_diff_current == 0] = 1e-8
